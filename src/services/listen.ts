  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        backfill_from: { type: 'string', description: 'Back-check window start (ISO, any precision — use this for hours/minutes, e.g. just-sent test mail). Scans the source from here against this new handler now.' },
        backfill_days: { type: 'number', description: 'Back-check convenience: scan the last N days (omit/0 = forward-only). Prefer backfill_from for sub-day windows. Response includes { from, fetched, scanned, tagged }.' },
      },
